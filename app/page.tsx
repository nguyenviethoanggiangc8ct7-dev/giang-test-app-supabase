  ...
  return (
    <div className="space-y-10">
      <header className="text-center py-12">
        <h1 className="text-4xl font-bold mb-3">Supabase Self-Hosted Demo</h1>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {features.map((f) => (
          ...
          >
            ...
            <span className="text-blue-600 text-sm font-medium">{f.cta} →</span>
          </Link>
        ))}
      </section>
      ...
      <section className="bg-yellow-50 border border-yellow-200 rounded-lg p-5 text-sm">
        ...
        <code className="bg-white px-1.5 py-0.5 rounded">schema.sql</code> trên
        Supabase Studio SQL Editor để tạo bảng <code>posts</code>,{" "}
        <code>profiles</code>,<code> messages</code> và bucket{" "}
        <code>avatars</code>.
      </section>
    </div>
  );
